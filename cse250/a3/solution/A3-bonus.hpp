...
private:
   ...
   // stores pointer to end of each list.
   std::vector<PairList::Node*> _listEnds;


   // store the length of each list.
   std::vector<int> _listLengths;
...
public:
   ...
   class Iterator {
      ...
      // Field to store length of each list associated with a key.
      std::vector<int> _listLengths;

      // Field to store current key iterator is located at.
      int _currentKey;

      // Field to store current level in lists iterator is located at.
      int _currentLevel;

   public:

      // The level flag is set to -1 to signify the reverse end.
      Iterator(PairList* pairList, bool atEnd, int level = 0);
      ...
      */
      ...
      // Decremenet the iterator (postfix)
      Iterator operator--(int);
